...
export default function TaskTimer({ taskId, plannedDuration, taskTitle }: TaskTimerProps) {
  ...
  if (!showTimer && !timerState.isRunning && !timerState.isPaused) {
    return (
      <button
        onClick={handleStart}
        className="inline-flex items-center gap-1 px-2 py-1 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded transition-all text-xs"
    ...
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 border border-gray-200 shadow-sm">
      ...
      <div className="flex gap-2">
        {!timerState.isRunning && !timerState.isPaused && (
          <button
            onClick={handleStart}
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg hover:from-emerald-600 hover:to-teal-600 transition-all text-sm font-medium"
          >
