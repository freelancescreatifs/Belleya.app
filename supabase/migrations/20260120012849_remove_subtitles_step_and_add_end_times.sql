/*
  # Retirer l'étape Sous-titres et ajouter les heures de fin

  1. Modifications
    - Supprimer les colonnes liées aux sous-titres (date_subtitles, date_subtitles_time)
    - Ajouter des colonnes pour les heures de fin de chaque étape
    - Mettre à jour le trigger pour ne plus créer de tâches pour les sous-titres

  2. Nouvelles colonnes
    - date_script_end_time
    - date_shooting_end_time
    - date_editing_end_time
    - date_scheduling_end_time
*/

-- Supprimer les colonnes de sous-titres
ALTER TABLE content_calendar DROP COLUMN IF EXISTS date_subtitles;
ALTER TABLE content_calendar DROP COLUMN IF EXISTS date_subtitles_time;

-- Ajouter les heures de fin pour chaque étape
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'content_calendar' AND column_name = 'date_script_end_time'
  ) THEN
    ALTER TABLE content_calendar ADD COLUMN date_script_end_time text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'content_calendar' AND column_name = 'date_shooting_end_time'
  ) THEN
    ALTER TABLE content_calendar ADD COLUMN date_shooting_end_time text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'content_calendar' AND column_name = 'date_editing_end_time'
  ) THEN
    ALTER TABLE content_calendar ADD COLUMN date_editing_end_time text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'content_calendar' AND column_name = 'date_scheduling_end_time'
  ) THEN
    ALTER TABLE content_calendar ADD COLUMN date_scheduling_end_time text;
  END IF;
END $$;

-- Mettre à jour la fonction determine_content_status pour retirer les sous-titres
...
-- Mettre à jour le trigger pour retirer l'étape sous-titres et ajouter les heures de fin
...
BEGIN
  ...
  -- Déterminer automatiquement le statut du contenu (sans sous-titres)
