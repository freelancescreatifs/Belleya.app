/*
  # Fix admin_delete_user: Remove redundant is_admin() check

  ## Problem
  - The Edge Function `admin-delete-user` calls this RPC using the Service Role Key
  - `is_admin()` relies on `auth.uid()`, which returns NULL for Service Role Key calls
  - This causes "Access denied: Admin privileges required" every time

  ## Fix
  - Remove the `is_admin()` check from this function
  - Authorization is already enforced by the Edge Function (validates JWT + checks user_roles)
  - The function is SECURITY DEFINER and only reachable via the Edge Function

  ## No other changes
  - All DELETE statements remain identical
