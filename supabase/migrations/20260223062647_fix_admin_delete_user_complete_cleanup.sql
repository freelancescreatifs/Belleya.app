/*
  # Fix admin_delete_user function - remove VIEW delete and add orphaned user handling

  1. Bug Fix
    - Remove `DELETE FROM public.public_provider_profiles` which is a VIEW, not a table
    - This was causing "cannot delete from view" 500 errors
    - The view auto-derives from company_profiles + user_profiles which are already deleted

  2. Improvement
    - Handle users who have NO user_profiles row (orphaned auth users)
    - Previously the function would silently fail for orphaned users since v_company_id lookup returned NULL
    - Now it safely proceeds even when user_profiles row doesn't exist
*/
